...
mean_step_runtime_us = NaN;
if simulation.controller_evaluation_count > 0
    mean_step_runtime_us = finite_product( ...
        simulation.controller_runtime_seconds / simulation.controller_evaluation_count, 1e6);
end
...
metrics = struct( ...
    ...
    "controller_runtime_seconds", simulation.controller_runtime_seconds, ...
    "mean_step_runtime_us", mean_step_runtime_us, ...
    "simulation_runtime_seconds", simulation.runtime_seconds, ...
    "controller_evaluation_count", simulation.controller_evaluation_count, ...
    "controller_completed_count", simulation.controller_completed_count);
end
...
simulation_fields = ["controller_name", "scenario_name", "seed", ...
    ...
    "controller_evaluation_count", "controller_completed_count"];
...
if (sample_count == 0 && simulation.success) || ...
        simulation.controller_completed_count > sample_count || ...
        (sample_count > 0 && simulation.controller_completed_count == 0)
    invalid_input();
end
end

function validate_lifecycle(simulation)
attempted = simulation.controller_evaluation_count;
completed = simulation.controller_completed_count;
elapsed = simulation.controller_runtime_seconds;
if ~is_valid_scalar(attempted) || attempted < 0 || fix(attempted) ~= attempted || ...
        ~is_valid_scalar(completed) || completed < 0 || fix(completed) ~= completed || ...
        completed > attempted || attempted - completed > 1 || ...
        ~is_valid_scalar(elapsed) || elapsed < 0 || elapsed > simulation.runtime_seconds || ...
        (attempted == 0 && elapsed ~= 0) || ...
        (simulation.success && (attempted == 0 || completed ~= attempted))
    invalid_input();
end
end
