...
validate_controller_log_time(timing.u_raw_time, final_time, timing.sample_time);
validate_controller_log_time(timing.u_time, final_time, timing.sample_time);
validate_controller_log_time( ...
    timing.controller_update_time, final_time, timing.sample_time);
if isfield(timing, "fuzzy_gain_time") && ~isempty(timing.fuzzy_gain_time)
    validate_controller_log_time( ...
        timing.fuzzy_gain_time, final_time, timing.sample_time);
end
end

function validate_controller_log_time(time, final_time, sample_time)
validate_time(time, "twsbr:equivalence:invalid_timing");
if abs(time(1)) > 1e-12 || time(end) < final_time - 1e-12 || ...
        any(abs(diff(time) - sample_time) > 1e-12)
    error("twsbr:equivalence:invalid_timing", ...
        "Controller logs must cover the comparison interval at sample time.");
end
end
