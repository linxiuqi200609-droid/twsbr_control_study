...
if ~isfield(simulation, "timing") || ...
        ...
        ~isfield(simulation.timing, "u_raw") || ...
        ~isfield(simulation.timing, "u")
...
raw_update_time = validate_controller_log_time( ...
    simulation.timing.u_raw_time, simulation.timing.u_raw, ...
    scenario.duration, config.sample_time, ...
    "legacy raw control log");
validate_controller_log_time( ...
    simulation.timing.u_time, simulation.timing.u, ...
    scenario.duration, config.sample_time, ...
...
simulation.timing = make_timing_metadata( ...
    simulation.timing.u_raw_time, simulation.timing.u_raw, ...
    simulation.timing.u_time, simulation.timing.u, raw_update_time, ...
    config.sample_time, []);
...
[simulation.diagnostics, fuzzy_gain_time] = diagnostics_for_controller( ...
    ...
    config.sample_time);
simulation.timing = make_timing_metadata( ...
    u_raw_time, u_raw, u_time, u, raw_update_time, config.sample_time, ...
    fuzzy_gain_time);
...
function timing = make_timing_metadata( ...
        u_raw_time, u_raw, u_time, u, update_time, sample_time, fuzzy_gain_time)
timing = struct( ...
